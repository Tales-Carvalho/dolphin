...
{
	int length = (int)strlen(s);
	int x = (int)(screen_width/2.0 - (length/2.0) * RF_CHAR_WIDTH);
	printString(s, x, y, z);
}
	...
	while (*text)
	{
		if (*text == '\n')
		{
			*t = 0;
			...
			y -= RF_CHAR_HEIGHT * 2.0f / bbHeight;
			x = start_x;
			t = temp;
		}
		else if (*text == '\r')
		...
		{
			//todo: add tabs every something like 4 * RF_CHAR_WIDTH
			*t = 0;
			int cpos = (int)strlen(temp);
			int newpos = (cpos + 4) & (~3);
			printString(temp, x, y, z);
			x = start_x + (RF_CHAR_WIDTH * newpos) * 2.0f / bbWidth;
			t = temp;
			*t++ = ' ';
		}
		else
